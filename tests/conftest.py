...
def app():
    ...
    # FIX: Enable MARS to prevent "Connection is busy" error in threads
    base_uri = app.config["SQLALCHEMY_DATABASE_URI"]
    if "MultipleActiveResultSets" not in base_uri:
        app.config["SQLALCHEMY_DATABASE_URI"] = (
            base_uri + "&MultipleActiveResultSets=True"
        )
...
def auth_headers(client, app, db):
    ...
    # FIX: Higher entropy for unique usernames
    unique_suffix = uuid.uuid4().hex[:12]
    unique_username = f"sup_{unique_suffix}"
    user = User(
